...
> ⚠️ **ЧАСТИЧНО ИСПРАВЛЕНО**. Eviction и rebuild происходят под `self._lock`, поэтому другой thread не может получить stale индексы — lock предотвращает concurrent доступ. Проблема существует только теоретически при async доступе без lock.
...
> ✅ **ИСПРАВЛЕНО**. Удалены одиночные слова ("тот", "та", "то", "это"). Оставлены только значимые фразы: "помнишь", "как мы говорили", "вернёмся к", "насчёт того", "по поводу" и т.д.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. `MemorySystem` оставлен как reference implementation. `MemoryAdapter` из bridge.py является основным.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. Архитектурная особенность multi-agent системы. Для простых запросов overhead существенный.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. Архитектурная проблема — требует серьёзного рефакторинга.
...
| `response_cache` | `agent.py:51` | Наивный MD5 cache без инвалидации, кэширует tool-call результаты | ⚠️ Оставлен |
...
| `MemorySystem` | `core/memory.py` | ~300 строк мёртвого кода | ⚠️ Оставлен как reference |
| `parsers.py` | `utils/parsers.py` | Deprecated в v6.0, но файл остался | ⚠️ Оставлен |
| `_log_operation()` | `controller.py:470` | Метод никогда не вызывается из других методов controller | ⚠️ Оставлен |
...
| `kill_process()` | `controller.py:198` | Метод существует, но **нет** `KillProcessTool` | ⚠️ Оставлен |
| `open_file()` | `controller.py:417` | Метод существует, но **нет** `OpenFileTool` | ⚠️ Оставлен |
...
| Pickle→JSON migration | `vector_store.py:314-324` | Одноразовый код навсегда | ⚠️ Оставлен |
...
| `except:` (bare) ×15+ | `app_finder.py` | 15+ bare `except:` проглатывают все ошибки | ⚠️ Оставлен |
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. Требует UI-решения (confirm dialog).
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. Требует UI-решения.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. `shell=False` уже предотвращает injection, но path validation не добавлен.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. Требует выбора единой системы логирования.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. Нужно добавить в requirements.txt.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. В asyncio single-thread модели проблема минимальна.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. Не критично — exception ловится в try/except внутри.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. Требует рефакторинга KG.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. Ограничено `FILE_SEARCH_MAX_DEPTH` и `FILE_SEARCH_MAX_RESULTS`, но всё ещё sync.
...
> ⚠️ **НЕ ИСПРАВЛЕНО**. Результат кэшируется после первого запуска.
...
|-----------|-------|------------|----------|
| Баги | 15 | 13 | 2 |
| Архитектурные проблемы | 10 | 6 | 4 |
| Заглушки / мёртвый код | 14 | 8 | 6 |
| Костыли | 13 | 7 | 6 |
| Безопасность | 6 | 2 | 4 |
| Совместимость | 5 | 2 | 3 |
